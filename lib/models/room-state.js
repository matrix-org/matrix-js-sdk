...
/**
 * Get the encryption configuration information from the room state
 *
 * @return {object} m.encryption data
 */
RoomState.prototype.getEncryption = function() {
    // the encryption data is in the m.room.create event.

    var createEvent = this.getStateEvents('m.room.create', '');
    if (!createEvent) {
        // this is nonsensical, but...
        return null;
    }

    return createEvent.getContent()['m.encryption'] || null;
};
