...
WebStorageStore.prototype.storeUser = function(user) {
    ...
    this.store.setItem("user_" + user.userId, {
        presence: user.events.presence
    });
};
...
WebStorageStore.prototype.getUser = function(userId) {
    var userData = this.store.getItem("user_" + userId);
    if (!userData) {
        return null;
    }
    var user = new User(userId);
    if (userData.presence) {
        user.setPresenceEvent(userData.presence);
    }
    return user;
};
...
WebStorageStore.prototype.scrollback = function(room, limit) {
    ...
    var i;
    var earliestIndex = this._tokens[room.storageToken].earliestIndex;
    var earliestEventId = room.timeline[0].getId();
    var batch = this.store.getItem(
        keyName(room.roomId, "timeline", earliestIndex)
    );
    if (!batch) {
    ...
    while (scrollback.length < limit) {
        earliestIndex--;
        batch = this.store.getItem(keyName(room.roomId, "timeline", earliestIndex));
        if (!batch) {
            // no more events
            break;
        }
