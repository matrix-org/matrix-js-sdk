...
/**
 * Add a new room to the client, and configure it according to the state.
 *
 * @param {module:models/room~Room} room room object to add
 * @fires module:client~MatrixClient#event:"Room"
 *
 */
SyncApi.prototype._addRoom = function(room) {
    this.client.store.storeRoom(room);
    this.client.emit("Room", room);
    var encryption = room.currentState.getEncryption();
    ...
    // TODO: it might be necessary to store the promise returned here, so that
    // we can queue outgoing messages until it has completed. Or maybe we could
    // store a promise for each recipient.

    var self = this;
    this.client.downloadKeys(users, true).then(function() {
        return self.client.setRoomEncryption(room.roomId, config);
    }).done();
};
