...
import { SummeryStats } from "../../../../src/webrtc/stats/summeryStats";
...
describe("GroupCallStats", () => {
    ...
    describe("should on start", () => {
        ...
        it("starting processing and calling the collectors", async () => {
            ...
            stats.reports.emitSummeryStatsReport = jest.fn();
            const summeryStats = {} as SummeryStats;
            let processStatsSpy;
            if (collector) {
                processStatsSpy = jest.spyOn(collector, "processStats").mockResolvedValue(summeryStats);
                stats.start();
                jest.advanceTimersByTime(TIME_INTERVAL);
            } else {
