...
export class MatrixBaseApis {
    ...
    async addThreePidOnly(data) {
        ...
        return this._http.authedRequest(undefined, "POST", path, null, data, { prefix });
    }
    /**
     ...
     *     an error response ({@link module:http-api.MatrixError}).
     */
    claimOneTimeKeys(devices, key_algorithm) {
        const queries = {};
    ...
    async requestEmailToken(email, clientSecret, sendAttempt, nextLink, callback, identityAccessToken) {
        ...
        try {
            const response = await this._http.idServerRequest(undefined, "POST", "/validate/email/requestToken", params, PREFIX_IDENTITY_V2, identityAccessToken);
            ...
            if (err.cors === "rejected" || err.httpStatus === 404) {
                ...
                return await this._http.idServerRequest(callback, "POST", "/validate/email/requestToken", params, PREFIX_IDENTITY_V1);
            }
            if (callback)
                callback(err);
            throw err;
    ...
    async requestMsisdnToken(phoneCountry, phoneNumber, clientSecret, sendAttempt, nextLink, callback, identityAccessToken) {
        ...
        try {
            const response = await this._http.idServerRequest(undefined, "POST", "/validate/msisdn/requestToken", params, PREFIX_IDENTITY_V2, identityAccessToken);
            // TODO: Fold callback into above call once v1 path below is removed
            if (callback)
                callback(null, response);
            return response;
            ...
            if (err.cors === "rejected" || err.httpStatus === 404) {
                ...
                // See https://github.com/vector-im/riot-web/issues/10443
                logger.warn("IS doesn't support v2, falling back to deprecated v1");
                return await this._http.idServerRequest(callback, "POST", "/validate/msisdn/requestToken", params, PREFIX_IDENTITY_V1);
            }
            if (callback)
                callback(err);
            throw err;
    ...
    async submitMsisdnToken(sid, clientSecret, msisdnToken, identityAccessToken) {
        ...
        try {
            return await this._http.idServerRequest(undefined, "POST", "/validate/msisdn/submitToken", params, PREFIX_IDENTITY_V2, identityAccessToken);
        }
        catch (err) {
            if (err.cors === "rejected" || err.httpStatus === 404) {
                // Fall back to deprecated v1 API for now
                ...
                // See https://github.com/vector-im/riot-web/issues/10443
                logger.warn("IS doesn't support v2, falling back to deprecated v1");
                return await this._http.idServerRequest(undefined, "POST", "/validate/msisdn/submitToken", params, PREFIX_IDENTITY_V1);
            }
            throw err;
        }
    }
    ...
    getIdentityHashDetails(identityAccessToken) {
        return this._http.idServerRequest(undefined, "GET", "/hash_details", null, PREFIX_IDENTITY_V2, identityAccessToken);
    }
    /**
     ...
     */
    async identityHashedLookup(addressPairs, // [["email@example.org", "email"], ["10005550000", "msisdn"]]
        identityAccessToken) {
        const params = {
        ...
        const response = await this._http.idServerRequest(undefined, "POST", "/lookup", params, PREFIX_IDENTITY_V2, identityAccessToken);
    ...
    async lookupThreePid(medium, address, callback, identityAccessToken) {
        try {
            ...
            if (err.cors === "rejected" || err.httpStatus === 404) {
                ...
                logger.warn("IS doesn't support v2, falling back to deprecated v1");
                return await this._http.idServerRequest(callback, "GET", "/lookup", params, PREFIX_IDENTITY_V1);
            }
            if (callback)
    ...
    async bulkLookupThreePids(query, identityAccessToken) {
        try {
            ...
            if (err.cors === "rejected" || err.httpStatus === 404) {
                ...
                logger.warn("IS doesn't support v2, falling back to deprecated v1");
                return await this._http.idServerRequest(undefined, "POST", "/bulk_lookup", params, PREFIX_IDENTITY_V1, identityAccessToken);
            }
            throw err;
        }
    }
    ...
    getIdentityAccount(identityAccessToken) {
        return this._http.idServerRequest(undefined, "GET", "/account", undefined, PREFIX_IDENTITY_V2, identityAccessToken);
    }
