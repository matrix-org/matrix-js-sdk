...
function keysFromRecoverySession(sessions, decryptionKey, roomId) {
    ...
    for (const [sessionId, sessionData] of Object.entries(sessions)) {
        try {
            const decrypted = keyFromRecoverySession(
                sessionData,
                decryptionKey
            );
...
export class MatrixClient extends EventEmitter {
    constructor(opts) {
        ...
        this.on("Event.decrypted", event => {
            ...
            const oldHighlight =
                oldActions && oldActions.tweaks
                    ? !!oldActions.tweaks.highlight
                    : false;
            const newHighlight =
        ...
        this.on("Room.receipt", (event, room) => {
            if (room && this.isRoomEncrypted(room.roomId)) {
                ...
                for (let i = events.length - 1; i >= 0; i--) {
                    ...
                    highlightCount += this.getPushActionsForEvent(event).tweaks
                        .highlight
                        ? 1
                        : 0;
                }
    ...
    getCapabilities(fresh = false) {
        ...
        return this._http
            .authedRequest(undefined, "GET", "/capabilities")
            .catch(e => {
                logger.error(e);
    ...
    async initCrypto() {
        ...
        if (!this._sessionStore) {
            ...
            throw new Error(
                `Cannot enable encryption: no sessionStore provided`
            );
        }
        if (!this._cryptoStore) {
     ...
     */
    async getStoredDevicesForUser(userId) {
        if (this._crypto === null) {
            throw new Error("End-to-end encryption disabled");
        }
     ...
     */
    async getStoredDevice(userId, deviceId) {
        if (this._crypto === null) {
            throw new Error("End-to-end encryption disabled");
        }
    ...
    setDeviceVerified(userId, deviceId, verified) {
        ...
        const prom = _setDeviceVerification(
            this,
            userId,
            deviceId,
            verified,
     ...
     * @fires module:client~event:MatrixClient"deviceVerificationChanged"
     */
    setDeviceBlocked(userId, deviceId, blocked) {
     ...
     * @fires module:client~event:MatrixClient"deviceVerificationChanged"
     */
    setDeviceKnown(userId, deviceId, known) {
        if (known === undefined) {
            known = true;
        }
        return _setDeviceVerification(
            this,
            userId,
            deviceId,
            null,
     ...
     */
    async checkEventSenderTrust(event) {
        const device = await this.getEventSenderDeviceInfo(event);
        if (!device) {
            return 0;
        }
        return await this._crypto.checkDeviceTrust(
            event.getSender(),
            device.deviceId
        );
    }
    ...
    async createKeyBackupVersion(info) {
        ...
        if (
            ...
            await this._crypto._crossSigningInfo.signObject(
                data.auth_data,
                "master"
            );
        }
    ...
    deleteKeyBackupVersion(version) {
        ...
        if (
            this._crypto.backupInfo &&
            this._crypto.backupInfo.version === version
        ) {
    ...
    async restoreKeyBackupWithSecretStorage(
        ...
        const privKey = decodeBase64(
            await this.getSecret("m.megolm_backup.v1")
        );
        return this._restoreKeyBackup(
            privKey,
            targetRoomId,
    ...
    _restoreKeyBackup(privKey, targetRoomId, targetSessionId, backupInfo) {
        if (this._crypto === null) {
            throw new Error("End-to-end encryption disabled");
        }
        let totalKeyCount = 0;
    ...
    getVisibleRooms() {
        ...
        for (const r of allRooms) {
            const createEvent = r.currentState.getStateEvents(
                "m.room.create",
                ""
            );
        ...
        return allRooms.filter(r => {
            const tombstone = r.currentState.getStateEvents(
                "m.room.tombstone",
                ""
            );
    ...
    getIgnoredUsers() {
        ...
        if (
            !event ||
            !event.getContent() ||
            !event.getContent()["ignored_users"]
        )
            return [];
    ...
    joinRoom(roomIdOrAlias, opts, callback) {
        ...
        const prom = new Promise((resolve, reject) => {
            sign_promise
                .then(function(signed_invite_object) {
                    const data = {};
    ...
    cancelPendingEvent(event) {
        if (
            [EventStatus.QUEUED, EventStatus.NOT_SENT].indexOf(event.status) < 0
        ) {
            throw new Error(
    ...
    sendEvent(roomId, eventType, content, txnId, callback) {
        return this._sendCompleteEvent(
            roomId,
            {
                type: eventType,
                content: content
     ...
     * @return {module:http-api.MatrixError} Rejects: with an error response.
     */
    _sendCompleteEvent(roomId, eventObject, txnId, callback) {
        ...
        logger.log(
            `sendEvent of type ${type} in ${roomId} with txnId ${txnId}`
        );
     ...
     * @return {module:http-api.MatrixError} Rejects: with an error response.
     */
    redactEvent(roomId, eventId, txnId, callback) {
        return this._sendCompleteEvent(
            roomId,
            {
    ...
    async sendReadReceipt(event, opts, callback) {
        ...
        if (room && room.hasPendingEvent(eventId)) {
            throw new Error(
                `Cannot set read receipt to a pending event (${eventId})`
            );
        }
        const addlContent = {
    ...
    async setRoomReadMarkers(roomId, rmEventId, rrEvent, opts) {
        ...
        if (room && room.hasPendingEvent(rmEventId)) {
            throw new Error(
                `Cannot set read marker to a pending event (${rmEventId})`
            );
        }
        ...
        if (rrEvent) {
            ...
            if (room && room.hasPendingEvent(rrEventId)) {
                throw new Error(
                    `Cannot set read receipt to a pending event (${rrEventId})`
                );
            }
            if (room) {
                room._addLocalEchoReceipt(
    ...
    getRoomUpgradeHistory(roomId, verifyLinks = false) {
        ...
        let createEvent = currentRoom.currentState.getStateEvents(
            "m.room.create",
            ""
        );
        while (createEvent) {
            ...
            if (predecessor && predecessor["room_id"]) {
                ...
                if (verifyLinks) {
                    const tombstone = refRoom.currentState.getStateEvents(
                        "m.room.tombstone",
                        ""
                    );
                    if (
                ...
                createEvent = refRoom.currentState.getStateEvents(
                    "m.room.create",
                    ""
                );
            } else {
        ...
        let tombstoneEvent = currentRoom.currentState.getStateEvents(
            "m.room.tombstone",
            ""
        );
        while (tombstoneEvent) {
            const refRoom = this.getRoom(
            ...
            currentRoom = refRoom;
            tombstoneEvent = currentRoom.currentState.getStateEvents(
                "m.room.tombstone",
                ""
            );
        }
    ...
    forget(roomId, deleteRoom, callback) {
        ...
        const promise = _membershipChange(
            this,
            roomId,
            undefined,
            "forget",
    ...
    _unstable_setStatusMessage(newMessage) {
        ...
        return Promise.all(
            this.getRooms().map(room => {
                ...
                const maySend = room.currentState.mayClientSendStateEvent(
                    type,
                    this
                );
                if (!maySend) {
     ...
     * @return {module:http-api.MatrixError} Rejects: with an error response.
     */
    inviteToPresenceList(callback, userIds) {
     ...
     * @return {module:http-api.MatrixError} Rejects: with an error response.
     **/
    dropFromPresenceList(callback, userIds) {
    ...
    scrollback(room, limit, callback) {
        ...
        const prom = new Promise((resolve, reject) => {
            ...
            sleep(timeToWaitMs)
                ...
                .then(
                    function(res) {
                        const matrixEvents = utils.map(
                            res.chunk,
                            _PojoToMatrixEventMapper(self)
                        );
                        if (res.state) {
                            ...
                            room.currentState.setUnknownStateEvents(
                                stateEvents
                            );
                        }
    ...
    getEventTimeline(timelineSet, eventId) {
        ...
        const self = this;
        const promise = self._http
            .authedRequest(undefined, "GET", path, params)
            .then(function(res) {
                if (!res.event) {
                    throw new Error(
                ...
                let timeline = timelineSet.getTimelineForEvent(
                    matrixEvents[0].getId()
                );
                if (!timeline) {
                    timeline = timelineSet.addTimeline();
                    timeline.initialiseState(
                        utils.map(res.state, self.getEventMapper())
                    );
                    timeline.getState(EventTimeline.FORWARDS).paginationToken =
                        res.end;
                } else {
                    const stateEvents = utils.map(
                        res.state,
                    ...
                    timeline
                        ...
                        .setUnknownStateEvents(stateEvents);
                }
    ...
    paginateEventTimeline(eventTimeline, opts) {
        const isNotifTimeline =
            eventTimeline.getTimelineSet() === this._notifTimelineSet;
        // TODO: we should implement a backoff (as per scrollback()) to deal more
        // nicely with HTTP errors.
        opts = opts || {};
        ...
        if (isNotifTimeline) {
            ...
            promise = this._http
                .authedRequest(undefined, "GET", path, params, undefined)
                .then(function(res) {
                    ...
                    for (let i = 0; i < res.notifications.length; i++) {
                        ...
                        event.setPushActions(
                            PushProcessor.actionListToActionsObject(
     ...
     * @return {Promise} Resolves: As requestEmailToken
     */
    async _requestTokenFromEndpoint(endpoint, params) {
        const postParams = Object.assign({}, params);
        // If the HS supports separate add and bind, then requestToken endpoints
        // don't need an IS as they are all validated by the HS directly.
        if (
            !(await this.doesServerSupportSeparateAddAndBind()) &&
            this.idBaseUrl
        ) {
    ...
    setRoomMutePushRule(scope, roomId, mute) {
        ...
        if (!mute) {
            ...
            if (hasDontNotifyRule) {
                deferred = this.deletePushRule(
                    scope,
                    "room",
                    roomPushRule.rule_id
                );
            }
        } else {
            if (!roomPushRule) {
                ...
                this.deletePushRule(scope, "room", roomPushRule.rule_id).then(
                    function() {
     ...
     * @private
     */
    _processRoomEventsSearch(searchResults, response) {
        ...
        for (let i = 0; i < room_events.results.length; i++) {
            const sr = SearchResult.fromJson(
                room_events.results[i],
                this.getEventMapper()
            );
    ...
    createFilter(content) {
        ...
        return this._http
            .authedRequest(undefined, "POST", path, undefined, content)
            .then(function(response) {
                // persist the filter
    ...
    getFilter(userId, filterId, allowCached) {
        ...
        return this._http
            .authedRequest(undefined, "GET", path, undefined, undefined)
            .then(function(response) {
                // persist the filter
    ...
    getOrCreateFilter(filterName, filter) {
        ...
        if (filterId) {
            ...
            promise = self
                .getFilter(self.credentials.userId, filterId, true)
                .then(
        ...
        return promise.then(function(existingId) {
            ...
            // create a new filter
            return self
                .createFilter(filter.getDefinition())
                .then(function(createdFilter) {
                    // debuglog("Created new filter ID %s: %s", createdFilter.filterId,
                    //          JSON.stringify(createdFilter.getDefinition()));
                    self.store.setFilterIdByName(
                        filterName,
                        createdFilter.filterId
                    );
    ...
    async doesServerAcceptIdentityAccessToken() {
        ...
        const unstableFeatures = response["unstable_features"];
        return (
            (versions && versions.includes("r0.6.0")) ||
    ...
    async relations(roomId, eventId, relationType, eventType, opts = {}) {
        const fetchedEventType = _getEncryptedIfNeededEventType(
            this,
            roomId,
            eventType
        );
        ...
        if (fetchedEventType === "m.room.encrypted") {
            const allEvents = originalEvent
                ? events.concat(originalEvent)
                : events;
            await Promise.all(
                allEvents.map(e => {
                    return new Promise(resolve =>
                        e.once("Event.decrypted", resolve)
                    );
                })
            );
...
function _sendEvent(client, room, event, callback) {
    ...
    return Promise.resolve()
        .then(function() {
            const encryptionPromise = _encryptEventIfNeeded(
                client,
                event,
                room
            );
        ...
        .then(function() {
            ...
            if (client.scheduler) {
                ...
                if (
                    promise &&
                    client.scheduler.getQueueForEvent(event).length > 1
                ) {
...
function _sendEventHttpRequest(client, event) {
    ...
    return client._http
        ...
        .then(res => {
            logger.log(
                `Event sent to ${event.getRoomId()} with event id ${
                    res.event_id
                }`
            );
            return res;
...
function setupCallEventHandler(client) {
    ...
    function callEventHandler(event) {
        const content = event.getContent();
        let call = content.call_id
            ? client.callList[content.call_id]
            : undefined;
        let i;
        ...
        if (event.getType() === "m.call.invite") {
            ...
            if (existingCall) {
                ...
                if (
                    existingCall.state === "wait_local_media" ||
