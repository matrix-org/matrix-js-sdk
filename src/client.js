...
export class MatrixClient extends EventEmitter {
     ...
     */
    async prepareKeyBackupVersion(password, { secureSecretStorage = false } = {}) {
        if (this._crypto === null) {
            throw new Error("End-to-end encryption disabled");
        }
     ...
     * key counts.
     */
    async restoreKeyBackupWithPassword(password, targetRoomId, targetSessionId, backupInfo) {
     ...
     * key counts.
     */
    async restoreKeyBackupWithSecretStorage(backupInfo, targetRoomId, targetSessionId) {
     ...
     * key counts.
     */
    restoreKeyBackupWithRecoveryKey(recoveryKey, targetRoomId, targetSessionId, backupInfo) {
    ...
    **/
    ...
    isSynapseAdministrator() {
        ...
        return this._http.authedRequest(undefined, 'GET', path, undefined, undefined, { prefix: '' }).then(r => r['admin']); // pull out the specific boolean we want
    };
 ...
 * Either let MatrixBaseApis implements EventEmitter therefore MatrixClient becomes a subclass of EventEmitter.
 */
