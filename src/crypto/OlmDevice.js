...
export class OlmDevice {
     ...
     * @private
     */
    _getSession(deviceKey, sessionId, txn, func) {
     ...
     * @private
     */
    _unpickleSession(sessionInfo, func) {
     ...
     * @private
     */
    _saveSession(deviceKey, sessionInfo, txn) {
            ...
            txn => {
                this._getAccount(txn, account => {
                    ...
                    try {
                        ...
                        this._storeAccount(txn, account);
                        const payloadString = session.decrypt(
                            messageType,
                            ciphertext
                        );
                        const sessionInfo = {
     ...
     */
    async getSessionIdForDevice(theirDeviceIdentityKey, nowait) {
     ...
     */
    async getSessionInfoForDevice(deviceIdentityKey, nowait) {
                ...
                this._cryptoStore.getEndToEndSessions(
                    ...
                    sessions => {
                        ...
                        for (const sessionId of sessionIds) {
                            this._unpickleSession(
                                sessions[sessionId],
                                sessInfo => {
                                    info.push({
                                        lastReceivedMessageTs:
                                            sessInfo.lastReceivedMessageTs,
            ...
            txn => {
                this._getSession(
                    ...
                    sessionInfo => {
                        ...
                        payloadString = sessionInfo.session.decrypt(
                            messageType,
                            ciphertext
                        );
        ...
        await this._cryptoStore.doTxn(
            ...
            txn => {
                this._getSession(
                    ...
                    txn,
                    sessionInfo => {
                        matches = sessionInfo.session.matches_inbound(
                            ciphertext
                        );
                    }
                );
    ...
    async recordSessionProblem(deviceKey, type, fixed) {
        await this._cryptoStore.storeEndToEndSessionProblem(
            deviceKey,
            type,
            fixed
        );
    ...
    async sessionMayHaveProblems(deviceKey, timestamp) {
        return await this._cryptoStore.getEndToEndSessionProblem(
            deviceKey,
            timestamp
        );
    }
     ...
     * @private
     */
    _saveOutboundGroupSession(session) {
     ...
     * @private
     */
    _getOutboundGroupSession(sessionId, func) {
     ...
     */
    _unpickleInboundGroupSession(sessionData, func) {
     ...
     * @private
     */
    _getInboundGroupSession(roomId, senderKey, sessionId, txn, func) {
     ...
     */
    async addInboundGroupSessionWithheld(
        roomId,
        senderKey,
        sessionId,
        code,
                ...
                this._getInboundGroupSession(
                    ...
                    (session, sessionData, withheld) => {
                        ...
                        try {
                            ...
                            if (
                                e &&
                                e.message === "OLM.UNKNOWN_MESSAGE_INDEX" &&
                                withheld
                            ) {
                                error = new algorithms.DecryptionError(
                                    "MEGOLM_UNKNOWN_INBOUND_SESSION_ID",
            ...
            txn => {
                this._getInboundGroupSession(
                    ...
                    (session, sessionData) => {
                        ...
                        const exportedSession = session.export_session(
                            chainIndex
                        );
