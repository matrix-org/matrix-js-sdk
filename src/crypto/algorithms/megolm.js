...
class OutboundSessionInfo {
    ...
    needsRotation(rotationPeriodMsgs, rotationPeriodMs) {
        ...
        if (this.useCount >= rotationPeriodMsgs ||
            ...
            logger.log("Rotating megolm session after " + this.useCount +
                " messages, " + sessionLifetime + "ms");
            return true;
        }
        return false;
    }
    markSharedWithDevice(userId, deviceId, chainIndex) {
        ...
        for (const userId in this.sharedWithDevices) {
            ...
            for (const deviceId in this.sharedWithDevices[userId]) {
                ...
                if (!devicesInRoom[userId].hasOwnProperty(deviceId)) {
                    logger.log("Starting new megolm session because we shared with " +
                        userId + ":" + deviceId);
                    return true;
                }
            }
...
class MegolmEncryption extends EncryptionAlgorithm {
     ...
     */
    async _ensureOutboundSession(devicesInRoom, blocked) {
        const self = this;
        let session;
        ...
        async function prepareSession(oldSession) {
            ...
            const errorDevices = [];
            await self._shareKeyWithDevices(session, shareMap, errorDevices);
     ...
     */
    _splitUserDeviceMap(session, chainIndex, devicemap, devicesByUser, errorDevices) {
     ...
     */
    _splitBlockedDevices(devicesByUser) {
        const maxUsersPerRequest = 20;
        // use an array where the slices of a content map gets stored
        let currentSlice = [];
     ...
     */
    _encryptAndSendKeysToDevices(session, chainIndex, userDeviceMap, payload) {
        const contentMap = {};
        const promises = [];
     ...
     *     for the given userDeviceMap is generated and has been sent.
     */
    async _sendBlockedNotificationsToDevices(session, userDeviceMap, payload) {
        const contentMap = {};
    ...
    async reshareKeyWithDevice(senderKey, sessionId, userId, device) {
        ...
        if (sentChainIndex === undefined) {
            logger.debug("megolm session ID " + sessionId + " never shared with device " +
        ...
        if (!key) {
            logger.warn(`No inbound session key found for megolm ${sessionId}: not re-sharing keys`);
            return;
        }
     ...
     *    olm session for
     */
    async _shareKeyWithDevices(session, devicesByUser, errorDevices) {
     ...
     */
    async _notifyBlockedDevices(session, devicesByUser) {
        const payload = {
            room_id: this._roomId,
            session_id: session.sessionId,
            algorithm: olmlib.MEGOLM_ALGORITHM,
     ...
     *   devices we should shared the session with.
     */
    _checkForUnknownDevices(devicesInRoom) {
        ...
        if (Object.keys(unknownDevices).length) {
            ...
            throw new UnknownDeviceError("This room contains unknown devices which have not been verified. " +
...
class MegolmDecryption extends DecryptionAlgorithm {
    ...
    async decryptEvent(event) {
        ...
        if (res === null) {
            ...
            throw new DecryptionError("MEGOLM_UNKNOWN_INBOUND_SESSION_ID", "The sender's device has not sent us the keys for this message.", {
                session: content.sender_key + '|' + content.session_id,
            });
        }
     ...
     * @param {module:models/event.MatrixEvent} event
     */
    _removeEventFromPendingList(event) {
        const content = event.getWireContent();
        const senderKey = content.sender_key;
        const sessionId = content.session_id;
        const senderPendingEvents = this._pendingEvents[senderKey];
    ...
    async onRoomKeyWithheldEvent(event) {
        ...
        if (content.code === "m.no_olm") {
            ...
            if (!device) {
                logger.info("Couldn't find device for identity key " + senderKey +
                    ": not establishing session");
                await this._olmDevice.recordSessionProblem(senderKey, "no_olm", false);
                this.retryDecryptionFromSender(senderKey);
                return;
            }
            await olmlib.ensureOlmSessionsForDevices(this._olmDevice, this._baseApis, { [sender]: [device] }, false);
            const encryptedContent = {
                algorithm: olmlib.OLM_ALGORITHM,
                sender_key: this._olmDevice.deviceCurve25519Key,
                ciphertext: {},
    ...
    shareKeysWithDevice(keyRequest) {
        ...
        this.olmlib.ensureOlmSessionsForDevices(this._olmDevice, this._baseApis, {
            ...
            logger.log("sharing keys for session " + body.sender_key + "|"
    ...
    async _buildKeyForwardingMessage(roomId, senderKey, sessionId) {
     ...
     */
    async _retryDecryption(senderKey, sessionId) {
        const senderPendingEvents = this._pendingEvents[senderKey];
