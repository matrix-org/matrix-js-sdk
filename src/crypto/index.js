...
import {
    InRoomChannel,
    InRoomRequests
} from "./verification/request/InRoomChannel";
import {
    ToDeviceChannel,
    ToDeviceRequests
} from "./verification/request/ToDeviceChannel";
...
export class Crypto extends EventEmitter {
    constructor(
        ...
        super();
        this._onDeviceListUserCrossSigningUpdated = this._onDeviceListUserCrossSigningUpdated.bind(
            this
        );
    ...
    async init() {
        ...
        this._deviceKeys[
            "ed25519:" + this._deviceId
        ] = this._olmDevice.deviceEd25519Key;
        this._deviceKeys[
            "curve25519:" + this._deviceId
        ...
        await this._cryptoStore.doTxn(
            ...
            txn => {
                this._cryptoStore.getCrossSigningKeys(txn, keys => {
                    if (keys) {
                        logger.log(
                            "Loaded cross-signing public keys from crypto store"
                        );
    ...
    async createRecoveryKeyFromPassphrase(password) {
        ...
        try {
            ...
            if (password) {
                ...
                keyInfo.pubkey = decryption.init_with_private_key(
                    derivation.key
                );
            } else {
    ...
    async bootstrapSecretStorage({
        ...
        try {
            const inStorage = await this._crossSigningInfo.isStoredInSecretStorage(
                this._secretStorage
            );
            if (!this._crossSigningInfo.getId() || !inStorage) {
                ...
                if (inStorage) {
                    logger.log(
                        "Cross-signing private keys found in secret storage"
                    );
                    ...
                    this._baseApis._cryptoCallbacks.saveCrossSigningKeys = keys =>
                        (crossSigningPrivateKeys = keys);
                    this._baseApis._cryptoCallbacks.getCrossSigningKey = name =>
                        crossSigningPrivateKeys[name];
                    await this.resetCrossSigningKeys(CrossSigningLevel.MASTER, {
                        authUploadDeviceSigningKeys
                    });
                }
            } else {
            ...
            if (!(await this.hasSecretStorageKey())) {
                ...
                if (keyBackupInfo) {
                    logger.log(
                        "Secret storage default key not found, using key backup key"
                    );
                    const opts = {
                    ...
                    logger.log(
                        "Secret storage default key not found, creating new key"
                    );
            ...
            if (Object.keys(crossSigningPrivateKeys).length) {
                logger.log(
                    "Storing cross-signing private keys in secret storage"
                );
    ...
    requestSecret(name, devices) {
        if (!devices) {
            devices = Object.keys(
                this._deviceList.getRawStoredDevicesForUser(this._userId)
            );
        }
    ...
    async resetCrossSigningKeys(
        ...
        try {
            ...
            await this._cryptoStore.doTxn(
                ...
                [IndexedDBCryptoStore.STORE_ACCOUNT],
                txn => {
                    this._cryptoStore.storeCrossSigningKeys(
                        txn,
                        this._crossSigningInfo.keys
                    );
                }
            ...
            // next time.
            logger.error(
                "Resetting cross-signing keys failed, revert to previous keys",
                e
            );
     ...
     * verifications, etc.
     */
    async _afterCrossSigningLocalKeyChange() {
        ...
        const users = {};
        for (const [userId, crossSigningInfo] of Object.entries(
        ...
        const shouldUpgradeCb = this._baseApis._cryptoCallbacks
        ...
        if (Object.keys(users).length > 0 && shouldUpgradeCb) {
            try {
                    ...
                    for (const userId of usersToUpgrade) {
                        if (userId in users) {
                            await this._baseApis.setDeviceVerified(
                                userId,
                                users[userId].crossSigningInfo.getId()
                            );
                        }
                    }
                }
                ...
                logger.log(
                    "shouldUpgradeDeviceVerifications threw an error: not upgrading",
                    e
                );
            }
        }
     ...
     */
    async _checkForDeviceVerificationUpgrade(userId, crossSigningInfo) {
        // only upgrade if this is the first cross-signing key that we've seen for
        // them, and if their cross-signing key isn't already verified
        const trustLevel = this._crossSigningInfo.checkUserTrust(
            crossSigningInfo
        );
        if (crossSigningInfo.firstUse && !trustLevel.verified) {
            ...
            const deviceIds = await this._checkForValidDeviceSignature(
                userId,
                crossSigningInfo.keys.master,
                devices
            );
            if (deviceIds.length) {
                return {
                    devices: deviceIds.map(deviceId =>
                        DeviceInfo.fromStorage(devices[deviceId], deviceId)
                    ),
                    crossSigningInfo
                };
            }
     ...
     *     to device info
     */
    async _checkForValidDeviceSignature(userId, key, devices) {
        const deviceIds = [];
    ...
    checkUserTrust(userId) {
        const userCrossSigning = this._deviceList.getStoredCrossSigningForUser(
            userId
        );
        if (!userCrossSigning) {
    ...
    async checkOwnCrossSigningTrust() {
        ...
        const newCrossSigning = this._deviceList.getStoredCrossSigningForUser(
            userId
        );
        if (!newCrossSigning) {
            logger.error(
                "Got cross-signing update event for user " +
                    userId +
        ...
        if (masterChanged) {
            ...
            try {
                ...
                if (!signing) {
                    throw new Error(
                        "Cross-signing master private key not available"
                    );
                }
            } finally {
                if (signing) signing.free();
            ...
            logger.info(
                "Got matching private key from callback for new public master key"
            );
        }
     ...
     */
    async _checkDeviceVerifications(userId) {
        if (this._crossSigningInfo.keys.user_signing) {
            const crossSigningInfo = this._deviceList.getStoredCrossSigningForUser(
                userId
            );
            if (crossSigningInfo) {
                const upgradeInfo = await this._checkForDeviceVerificationUpgrade(
    ...
    async _checkAndStartKeyBackup() {
        ...
        if (trustInfo.usable && !this.backupInfo) {
            ...
            if (backupInfo.version !== this.backupInfo.version) {
                ...
                logger.log(
                    "Backup version " + backupInfo.version + " still current"
                );
            }
        }
    ...
    async isKeyBackupTrusted(backupInfo) {
        ...
        if (backupInfo.auth_data.public_key === trustedPubkey) {
            logger.info(
                "Backup public key " + trustedPubkey + " is trusted locally"
            );
        ...
        for (const keyId of Object.keys(mySigs)) {
            ...
            if (crossSigningId === sigInfo.deviceId) {
                ...
                try {
                    ...
                    logger.warning(
                        "Bad signature from cross signing key " +
                            crossSigningId,
                        e
                    );
                    sigInfo.valid = false;
        ...
        ret.usable = ret.sigs.some(s => {
            return (
                s.valid &&
                ((s.device && s.deviceTrust.isVerified()) || s.crossSigningId)
            );
        });
    ...
    registerEventHandlers(eventEmitter) {
        const crypto = this;
        eventEmitter.on("RoomMember.membership", function(
            event,
            member,
            oldMembership
        ) {
    ...
    updateOneTimeKeyCount(currentCount) {
        if (isFinite(currentCount)) {
            ...
            throw new TypeError(
                "Parameter for updateOneTimeKeyCount has to be a number"
            );
        }
    }
    /**
    ...
    async setDeviceVerification(userId, deviceId, verified, blocked, known) {
        ...
        if (xsk && xsk.getId() === deviceId) {
            if (blocked !== null || known !== null) {
                throw new Error(
                    "Cannot set blocked or known for a cross-signing key"
                );
            }
            if (!verified) {
    ...
    requestVerificationDM(userId, roomId, methods) {
    ...
    requestVerification(userId, methods, devices) {
        if (!devices) {
            devices = Object.keys(
                this._deviceList.getRawStoredDevicesForUser(userId)
            );
        }
    ...
    async _requestVerificationWithChannel(
        userId,
        methods,
        channel,
        requestsMap
        ...
        if (methods) {
            verificationMethods = methods.reduce((map, name) => {
                ...
                if (!method) {
                    throw new Error(
                        `Verification method ${name} is not supported.`
                    );
                } else {
                    map.set(name, method);
                }
    ...
    async getOlmSessionsForUser(userId) {
        ...
        for (let j = 0; j < devices.length; ++j) {
            ...
            const sessions = await this._olmDevice.getSessionInfoForDevice(
                deviceKey
            );
    ...
    forceDiscardSession(roomId) {
        ...
        if (alg.forceDiscardSession === undefined) {
            throw new Error(
                "Room encryption algorithm doesn't support session discarding"
            );
        }
    ...
    async setRoomEncryption(roomId, config, inhibitDeviceQuery) {
        ...
        if (existingConfig) {
            if (JSON.stringify(existingConfig) != JSON.stringify(config)) {
                logger.error(
                    "Ignoring m.room.encryption event which requests " +
                        "a change of config in " +
                        roomId
                );
                return;
            }
        ...
        if (!existingConfig) {
            storeConfigPromise = this._roomList.setRoomEncryption(
                roomId,
                config
            );
        }
    ...
    trackRoomDevices(roomId) {
        const trackMembers = async () => {
            ...
            if (!room) {
                throw new Error(
                    `Unable to start tracking devices in unknown room ${roomId}`
                );
            }
        ...
        return Promise.all(
            keys.map(key => {
                if (!key.room_id || !key.algorithm) {
                    logger.warn(
                        "ignoring room key entry with missing fields",
                        key
                    );
                    return null;
                }
    ...
    async scheduleKeyBackupSend(maxDelay = 10000) {
        ...
        try {
            ...
            while (1) {
                ...
                try {
                    const numBackedUp = await this._backupPendingKeys(
     ...
     */
    async _backupPendingKeys(limit) {
        const sessions = await this._cryptoStore.getSessionsNeedingBackup(
            limit
        );
        if (!sessions.length) {
            return 0;
        ...
        for (const session of sessions) {
            ...
            const encrypted = this.backupKey.encrypt(
    ...
    async flagAllGroupSessionsForBackup() {
        await this._cryptoStore.doTxn(
            ...
            txn => {
                this._cryptoStore.getAllEndToEndInboundGroupSessions(
                    txn,
                    session => {
                        if (session !== null) {
                            this._cryptoStore.markSessionsNeedingBackup(
                                [session],
                                txn
                            );
                        }
    ...
    decryptEvent(event) {
        ...
        const alg = this._getRoomDecryptor(
            event.getRoomId(),
            content.algorithm
        );
    ...
    requestRoomKey(requestBody, recipients, resend = false) {
        return this._outgoingRoomKeyRequestManager
            .sendRoomKeyRequest(requestBody, recipients, resend)
            .catch(e => {
    ...
    cancelRoomKeyRequest(requestBody) {
        this._outgoingRoomKeyRequestManager
            .cancelRoomKeyRequest(requestBody)
            .catch(e => {
    ...
    async onCryptoEvent(event) {
        ...
        try {
            ...
            logger.error(
                "Error configuring encryption in room " + roomId + ":",
                e
            );
        }
    }
    ...
    async onSyncWillProcess(syncData) {
        if (!syncData.oldSyncToken) {
            ...
            logger.log(
                "Initial sync performed - resetting device tracking state"
            );
     ...
     * /keys/changes
     */
    async _evalDeviceListChanges(deviceLists) {
        ...
        if (
            deviceLists.left &&
            Array.isArray(deviceLists.left) &&
    ...
    _onToDeviceEvent(event) {
        try {
            logger.log(
                `received to_device ${event.getType()} from: ` +
                    `${event.getSender()} id: ${event.getId()}`
            );
            if (
                event.getType() == "m.room_key" ||
     ...
     */
    _onRoomKeyWithheldEvent(event) {
        const content = event.getContent();
        if (
            (content.code !== "m.no_olm" &&
                (!content.room_id || !content.session_id)) ||
            !content.algorithm ||
            !content.sender_key
        ) {
     ...
     */
    _onKeyVerificationMessage(event) {
        if (!ToDeviceChannel.validateEvent(event, this._baseApis)) {
            return;
        }
        const createRequest = event => {
            if (
                !ToDeviceChannel.canCreateRequest(
                    ToDeviceChannel.getEventType(event)
                )
            ) {
                return;
            }
            ...
            const channel = new ToDeviceChannel(this._baseApis, userId, [
                deviceId
            ]);
     ...
     */
    _onTimelineEvent(event, room, atStart, removed, { liveEvent } = {}) {
    ...
    async _handleVerificationEvent(
        event,
        requestsMap,
        createRequest,
        isLiveEvent = true
        ...
        try {
            ...
            logger.error(
                "error while handling verification event: " + err.message
            );
        }
        const shouldEmit =
            isNewRequest &&
     ...
     */
    async _onToDeviceBadEncrypted(event) {
        const content = event.getWireContent();
        ...
        const retryDecryption = () => {
            const roomDecryptors = this._getRoomDecryptors(
                olmlib.MEGOLM_ALGORITHM
            );
            for (const decryptor of roomDecryptors) {
        ...
        this._lastNewSessionForced[sender] =
            this._lastNewSessionForced[sender] || {};
        const lastNewSessionForced =
            this._lastNewSessionForced[sender][deviceKey] || 0;
        ...
        if (!device) {
            logger.info(
                "Couldn't find device for identity key " +
                    deviceKey +
        ...
        // to avoid races.
        const requestsToResend = await this._outgoingRoomKeyRequestManager.getOutgoingSentRoomKeyRequest(
            sender,
            device.deviceId
        );
     ...
     */
    _onRoomMembership(event, member, oldMembership) {
        ...
        if (this._roomDeviceTrackingState[roomId]) {
            if (member.membership == "join") {
                ...
                logger.log(
                    "Invite event for " + member.userId + " in " + roomId
                );
                this._deviceList.startTrackingDeviceList(member.userId);
            }
        }
     ...
     * @private
     */
    async _processReceivedRoomKeyRequests() {
        ...
        try {
            ...
            await Promise.all(
                requests.map(req => this._processReceivedRoomKeyRequest(req))
            );
            await Promise.all(
                cancellations.map(cancellation =>
                    this._processReceivedRoomKeyRequestCancellation(
                        cancellation
                    )
                )
            );
     ...
     */
    async _processReceivedRoomKeyRequest(req) {
        ...
        if (userId !== this._userId) {
            ...
            if (!device) {
                logger.debug(
                    `Ignoring keyshare for unknown device ${userId}:${deviceId}`
                );
                return;
            }
            try {
        ...
        if (!decryptor) {
            logger.log(
                `room key request for unknown alg ${alg} in room ${roomId}`
            );
            return;
        }
     ...
     */
    async _processReceivedRoomKeyRequestCancellation(cancellation) {
        logger.log(
     ...
     * unknown
     */
    _getRoomDecryptor(roomId, algorithm) {
        let decryptors;
        let alg;
        roomId = roomId || null;
        if (roomId) {
     ...
     */
    async _onDeviceListUserCrossSigningUpdated(userId) {
        if (userId === this._userId) {
...
function _maybeUploadOneTimeKeys(crypto) {
    ...
    const now = Date.now();
    if (
        crypto._lastOneTimeKeyCheck !== null &&
    ...
    function uploadLoop(keyCount) {
        ...
        return crypto._olmDevice
            .generateOneTimeKeys(keysThisLoop)
            .then(() => {
                return _uploadOneTimeKeys(crypto);
            })
            .then(res => {
                if (
                    res.one_time_key_counts &&
                    res.one_time_key_counts.signed_curve25519
                ) {
...
class IncomingRoomKeyRequest {
    constructor(event) {
        ...
        this.share = () => {
            throw new Error(
                "don't know how to share keys for this request yet"
            );
        };
    }
}
