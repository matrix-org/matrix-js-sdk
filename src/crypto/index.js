...
export class Crypto extends EventEmitter {
     ...
     */
    async bootstrapSecretStorage({ authUploadDeviceSigningKeys, createSecretStorageKey = async () => { }, keyBackupInfo, } = {}) {
     ...
     *     auth data as an object.
     */
    async resetCrossSigningKeys(level, { authUploadDeviceSigningKeys = async (func) => await func(), } = {}) {
     ...
     */
    async _storeTrustedSelfKeys(keys) {
        this._crossSigningInfo.setKeys(keys);
        await this._cryptoStore.doTxn(
            'readwrite', [IndexedDBCryptoStore.STORE_ACCOUNT],
            (txn) => {
    ...
    async _requestVerificationWithChannel(userId, methods, channel, requestsMap) {
    ...
    async backupGroupSession(roomId, senderKey, forwardingCurve25519KeyChain, sessionId, sessionKey, keysClaimed, exportFormat) {
        if (!this.backupInfo) {
    ...
    async flagAllGroupSessionsForBackup() {
        await this._cryptoStore.doTxn('readwrite', [
    ...
    async _handleVerificationEvent(event, requestsMap, createRequest, isLiveEvent = true) {
    ...
    async _processReceivedRoomKeyRequestCancellation(cancellation) {
        logger.log(`m.room_key_request cancellation for ${cancellation.userId}:` +
