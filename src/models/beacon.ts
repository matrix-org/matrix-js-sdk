...
export class Beacon extends TypedEventEmitter<Exclude<BeaconEvent, BeaconEvent.New>, BeaconEventHandlerMap> {
    ...
    constructor(private rootEvent: MatrixEvent) {
        super();
    ...
    private checkLiveness(): void {
        ...
        const startTimestamp =
            this._beaconInfo?.timestamp > Date.now()
                ? this._beaconInfo?.timestamp - 360000 /* 6min */
                : this._beaconInfo?.timestamp;
        this._isLive =
            !!this._beaconInfo?.live && isTimestampInDuration(startTimestamp, this._beaconInfo?.timeout, Date.now());
