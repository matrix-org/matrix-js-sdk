...
export class EventTimelineSet extends EventEmitter {
    ...
    getPendingEvents() {
        ...
        if (this._filter) {
            return this._filter.filterRoomTimeline(
    ...
    resetLiveTimeline(backPaginationToken, forwardPaginationToken) {
        ...
        const resetAllTimelines =
            !this._timelineSupport || !forwardPaginationToken;
        ...
        newTimeline.setPaginationToken(
            backPaginationToken,
    ...
    addEventsToTimeline(events, toStartOfTimeline, timeline, paginationToken) {
        ...
        const direction = toStartOfTimeline
            ? EventTimeline.BACKWARDS
            : EventTimeline.FORWARDS;
        const inverseDirection = toStartOfTimeline
            ? EventTimeline.FORWARDS
            : EventTimeline.BACKWARDS;
        ...
        for (let i = 0; i < events.length; i++) {
            ...
            if (existingTimeline == timeline) {
                debuglog(
                    "Event " + eventId + " already in timeline " + timeline
                );
                continue;
            }
            ...
            existingTimeline.setNeighbouringTimeline(
                timeline,
                inverseDirection
            );
            timeline = existingTimeline;
     ...
     */
    addLiveEvent(event, duplicateStrategy, fromCache) {
        if (this._filter) {
        ...
        if (timeline) {
            if (duplicateStrategy === "replace") {
                debuglog(
                    "EventTimelineSet.addLiveEvent: replacing duplicate event " +
                        event.getId()
                );
     ...
     * @fires module:client~MatrixClient#event:"Room.timeline"
     */
    addEventToTimeline(event, timeline, toStartOfTimeline, fromCache) {
        const eventId = event.getId();
    ...
    aggregateRelations(event) {
        ...
        let relatesToEvent;
        if (!relationsWithEventType) {
            relationsWithEventType = relationsWithRelType[
                eventType
            ] = new Relations(relationType, eventType, this.room);
            isNewRelations = true;
