...
class ThreadedReceipts {
    ...
    ///**
    // * Find the latest threaded receipt of the specified type for the supplied user in the supplied thread.
    // *
    // * @returns the found receipt info or undefined if we don't have one.
    // */
    //public getByType(threadId: string, userId: string, synthetic: boolean): ReceiptInfo | undefined {
    //    return this.data.get(threadId)?.getByType(userId, synthetic);
    //}
}
