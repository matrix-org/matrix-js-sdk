...
export class RoomState extends EventEmitter {
    constructor(roomId, oobMemberFlags = undefined) {
        super();
        this.roomId = roomId;
        this.members = {
            // userId: RoomMember
    ...
    setUnknownStateEvents(events) {
        const unknownStateEvents = events.filter(event => {
            return (
                this.events[event.getType()] === undefined ||
     ...
     */
    _getOrCreateMember(userId, event) {
    ...
    getUserIdsWithDisplayName(displayName) {
        return (
            this._displayNameToUserIds[utils.removeHiddenChars(displayName)] ||
            []
        );
    }
    /**
     ...
     */
    _hasSufficientPowerLevelFor(action, powerLevel) {
     ...
     *                        according to the room's state.
     */
    _maySendEventOfType(eventType, userId, state) {
        const power_levels_event = this.getStateEvents(
            "m.room.power_levels",
            ""
        );
        let power_levels;
        ...
        if (power_levels_event) {
            ...
            const userPowerLevel =
                power_levels.users && power_levels.users[userId];
    ...
    mayTriggerNotifOfType(notifLevelKey, userId) {
        ...
        if (
            ...
            notifLevel = powerLevelsEvent.getContent().notifications[
                notifLevelKey
            ];
        }
...
function _updateDisplayNameCache(roomState, userId, displayName) {
    ...
    if (oldName) {
        ...
        const existingUserIds =
            roomState._displayNameToUserIds[strippedOldName];
        if (existingUserIds) {
    ...
    const strippedDisplayname =
        displayName && utils.removeHiddenChars(displayName);
