...
export class Room extends EventEmitter {
    constructor(roomId, client, myUserId, opts) {
        ...
        opts.pendingEventOrdering =
            opts.pendingEventOrdering || "chronological";
        this.reEmitter = new ReEmitter(this);
        if (
            ["chronological", "detached"].indexOf(opts.pendingEventOrdering) ===
            -1
        ) {
            throw new Error(
     ...
     */
    getVersion() {
        const createEvent = this.currentState.getStateEvents(
            "m.room.create",
            ""
        );
        if (!createEvent) {
            logger.warn(
                "Room " + this.room_id + " does not have an m.room.create event"
            );
            return "1";
        }
    ...
    async getRecommendedVersion() {
        ...
        if (result.urgent && result.needsUpgrade) {
            ...
            if (!versionCap) {
                logger.warn(
                    "No room version capability - assuming upgrade required."
                );
                return result;
            } else {
    ...
    _checkVersionAgainstCapability(versionCap) {
        ...
        const stableVersions = Object.keys(versionCap.available).filter(
            v => versionCap.available[v] === "stable"
        );
    ...
    guessDMUserId() {
        ...
        const hasHeroes =
            Array.isArray(this._summaryHeroes) && this._summaryHeroes.length;
        if (hasHeroes) {
            return this._summaryHeroes[0];
        }
    ...
    getAvatarFallbackMember() {
        ...
        const hasHeroes =
            Array.isArray(this._summaryHeroes) && this._summaryHeroes.length;
        if (hasHeroes) {
            const availableMember = this._summaryHeroes
                .map(userId => {
        ...
        if (hasHeroes) {
            const availableUser = this._summaryHeroes
                .map(userId => {
    ...
    async _loadMembers() {
        ...
        let rawMembersEvents = await this._client.store.getOutOfBandMembers(
            this.roomId
        );
        if (rawMembersEvents === null) {
            ...
            logger.log(
                `LL: got ${rawMembersEvents.length} ` +
                    `members from server for room ${this.roomId}`
            );
        }
        const memberEvents = rawMembersEvents.map(
            this._client.getEventMapper()
        );
    ...
    loadMembersIfNeeded() {
        ...
        const inMemoryUpdate = this._loadMembers()
            .then(result => {
                this.currentState.setOutOfBandMembers(result.memberEvents);
                // now the members are loaded, start to track the e2e devices if needed
                if (
                    this._client.isCryptoEnabled() &&
                    this._client.isRoomEncrypted(this.roomId)
                ) {
        ...
        inMemoryUpdate
            .then(fromServer => {
                if (fromServer) {
                    ...
                    logger.log(
                        `LL: telling store to write ${oobMembers.length}` +
                    ...
                    return (
                        store
                            ...
                            .catch(err => {
                                logger.log(
                                    "LL: storing OOB room members failed, oh well",
                                    err
                                );
                            })
                    );
     ...
     */
    _cleanupAfterLeaving() {
        this.clearLoadedMembersIfNeeded().catch(err => {
            logger.error(
                `error after clearing loaded members from ` +
    ...
    _fixUpLegacyTimelineFields() {
        ...
        this.oldState = this.getLiveTimeline().getState(
    ...
    async hasUnverifiedDevices() {
        ...
        for (const member of e2eMembers) {
            const devices = await this._client.getStoredDevicesForUser(
                member.userId
            );
     ...
     */
    getAvatarUrl(baseUrl, width, height, resizeMethod, allowDefault) {
        const roomAvatarEvent = this.currentState.getStateEvents(
            "m.room.avatar",
            ""
        );
        ...
        const mainUrl = roomAvatarEvent
            ? roomAvatarEvent.getContent().url
            : null;
        if (mainUrl) {
            ...
            return getIdenticonUri(baseUrl, this.roomId, width, height);
        }
        return null;
    }
    /**
        ...
        if (aliasEvents) {
            for (let i = 0; i < aliasEvents.length; ++i) {
                ...
                if (utils.isArray(aliasEvent.getContent().aliases)) {
                    const filteredAliases = aliasEvent
                        .getContent()
                        .aliases.filter(a => {
                            if (typeof a !== "string") return false;
                            if (a[0] !== "#") return false;
                            if (!a.endsWith(`:${aliasEvent.getStateKey()}`))
                                return false;
     ...
     */
    getCanonicalAlias() {
        const canonicalAlias = this.currentState.getStateEvents(
    ...
    shouldEncryptForInvitedMembers() {
        const ev = this.currentState.getStateEvents(
        ...
        return (
            ev &&
            ev.getContent() &&
            ev.getContent().history_visibility !== "joined"
        );
    }
    /**
    ...
    getOrCreateFilteredTimelineSet(filter) {
        ...
        while (timeline.getNeighbouringTimeline(EventTimeline.BACKWARDS)) {
            timeline = timeline.getNeighbouringTimeline(
                EventTimeline.BACKWARDS
            );
        }
        timelineSet
     ...
     * @private
     */
    _addLiveEvent(event, duplicateStrategy, fromCache) {
        if (event.isRedaction()) {
            const redactId = event.event.redacts;
            // if we know about this event, redact its contents now.
            const redactedEvent = this.getUnfilteredTimelineSet().findEventById(
                redactId
            );
            if (redactedEvent) {
        ...
        if (event.getUnsigned().transaction_id) {
            const existingEvent = this._txnToEvent[
        ...
        for (let i = 0; i < this._timelineSets.length; i++) {
            this._timelineSets[i].addLiveEvent(
                event,
                duplicateStrategy,
                fromCache
            );
        ...
        if (event.sender && event.getType() !== "m.room.redaction") {
            this.addReceipt(
                synthesizeReceipt(event.sender.userId, event, "m.read"),
                true
            );
    ...
    addPendingEvent(event, txnId) {
        if (event.status !== EventStatus.SENDING) {
            throw new Error(
                "addPendingEvent called on an event with status " + event.status
            );
        }
        if (this._txnToEvent[txnId]) {
            throw new Error(
                "addPendingEvent called on an event with known txnId " + txnId
            );
        }
        ...
        if (this._opts.pendingEventOrdering == "detached") {
            if (
                this._pendingEventList.some(
                    e => e.status === EventStatus.NOT_SENT
                )
            ) {
                logger.warn(
                    "Setting event as NOT_SENT due to messages in the same state"
                );
            ...
            if (event.isRedaction()) {
                ...
                if (!redactedEvent) {
                    redactedEvent = this.getUnfilteredTimelineSet().findEventById(
                        redactId
                    );
                }
                if (redactedEvent) {
            ...
            for (let i = 0; i < this._timelineSets.length; i++) {
                ...
                if (timelineSet.getFilter()) {
                    if (
                        timelineSet.getFilter().filterRoomTimeline([event])
                            .length
                    ) {
     ...
     */
    _aggregateNonLiveRelation(event) {
        ...
        for (let i = 0; i < this._timelineSets.length; i++) {
            ...
            if (timelineSet.getFilter()) {
                if (
                    timelineSet.getFilter().filterRoomTimeline([event]).length
                ) {
     ...
     * @private
     */
    _handleRemoteEcho(remoteEvent, localEvent) {
    ...
    updatePendingEvent(event, newStatus, newEventId) {
        ...
        if (newStatus == EventStatus.SENT && !newEventId) {
            throw new Error(
                "updatePendingEvent called with status=SENT, " +
                    "but no new event id"
            );
        }
        // SENT races against /sync, so we have to special-case it.
        if (newStatus == EventStatus.SENT) {
            const timeline = this.getUnfilteredTimelineSet().eventIdToTimeline(
                newEventId
            );
            if (timeline) {
        ...
        if (!oldStatus) {
            throw new Error(
                "updatePendingEventStatus called on an event which is " +
                    "not a local echo."
            );
        }
        ...
        if (!allowed || allowed.indexOf(newStatus) < 0) {
            throw new Error(
                "Invalid EventStatus transition " + oldStatus + "->" + newStatus
            );
        }
        ...
        if (newStatus == EventStatus.SENT) {
            ...
            if (this._pendingEventList) {
                const idx = this._pendingEventList.findIndex(
                    ev => ev.getId() === oldEventId
                );
                if (idx !== -1) {
                    const [removedEvent] = this._pendingEventList.splice(
                        idx,
                        1
                    );
    ...
    _revertRedactionLocalEcho(redactionEvent) {
     ...
     */
    addLiveEvents(events, duplicateStrategy, fromCache) {
        let i;
        if (
            duplicateStrategy &&
            ["replace", "ignore"].indexOf(duplicateStrategy) === -1
        ) {
            throw new Error(
                "duplicateStrategy MUST be either 'replace' or 'ignore'"
            );
        }
    ...
    recalculate() {
        ...
        if (
            membershipEvent &&
            membershipEvent.getContent().membership === "invite"
        ) {
            const strippedStateEvents =
                membershipEvent.event.invite_room_state || [];
            utils.forEach(strippedStateEvents, function(strippedEvent) {
                const existingEvent = self.currentState.getStateEvents(
                    strippedEvent.type,
                    strippedEvent.state_key
                );
                if (!existingEvent) {
                    // set the fake stripped event instead
                    self.currentState.setStateEvents([
                        new MatrixEvent({
    ...
    _addReceiptsToStructure(event, receipts) {
        ...
        utils.keys(event.getContent()).forEach(function(eventId) {
            utils
                .keys(event.getContent()[eventId])
                .forEach(function(receiptType) {
                    utils
                        .keys(event.getContent()[eventId][receiptType])
                        .forEach(function(userId) {
                            const receipt = event.getContent()[eventId][
                                receiptType
                            ][userId];
...
function calculateRoomName(room, userId, ignoreRoomNameEvent) {
    if (!ignoreRoomNameEvent) {
        ...
        if (
            mRoomName &&
            mRoomName.getContent() &&
            mRoomName.getContent().name
        ) {
    ...
    if (myMembership == "join") {
        const thirdPartyInvites = room.currentState.getStateEvents(
