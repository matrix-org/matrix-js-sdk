...
import { normalize, noUnsafeEventProps, removeElement, sleep } from "../utils.ts";
import {
    type IEvent,
...
export class Room extends ReadReceipt<RoomEmittedEvents, RoomEventHandlerMap> {
    ...
    /**
     * Get the room's effective message retention `max_lifetime` (in ms), per
     * [MSC1763](https://github.com/matrix-org/matrix-spec-proposals/pull/1763).
     *
     * Reads the room's `m.room.retention` state event if present; otherwise falls back
     * to the server's advertised default (see {@link MatrixClient.setServerDefaultMaxLifetime}).
     *
     * @returns The max lifetime in ms, or `null` if no retention policy applies.
     */
    public getMaxLifetime(): number | null {
        const retentionEvent = this.currentState.getStateEvents(EventType.RoomRetention, "");
        const stateMaxLifetime = retentionEvent?.getContent<{ max_lifetime?: unknown }>().max_lifetime;
        if (typeof stateMaxLifetime === "number" && stateMaxLifetime > 0) {
            return stateMaxLifetime;
        }
        const serverDefault = this.client.getServerDefaultMaxLifetime();
        if (typeof serverDefault === "number" && serverDefault > 0) {
    ...
    /**
     * Synthesize a local redaction event for `event` because the room's MSC1763
     * retention policy says it has expired.
     *
     * The synthesized event isn't sent to the server: it only exists in-memory so
     * that downstream code can treat the event as redacted.
     */
    private makeRetentionRedaction(event: MatrixEvent): MatrixEvent {
        const eventId = event.getId()!;
        return new MatrixEvent({
            type: EventType.RoomRedaction,
            // No real sender — this redaction was generated by the local client. Use the
            // event's own sender so authorization checks downstream don't reject it.
            sender: event.getSender() ?? undefined,
            redacts: eventId,
            content: { reason: "Client-side retention policy expiry (MSC1763)" },
            // Synthetic event id; the `~` prefix keeps it distinguishable from server-issued ids.
            event_id: `~retention:${eventId}`,
    ...
    /**
     * Walk `events` and locally redact any whose age exceeds the room's retention
     * `max_lifetime` (MSC1763). Events that are already redacted, redactions themselves,
     * and state events are skipped.
     *
     * Intended to be called whenever events enter the client — from the sync accumulator
     * cache, from /sync, or from /messages.
     */
    public applyRetentionPolicy(events: Iterable<MatrixEvent>): void {
        const maxLifetime = this.getMaxLifetime();
        if (maxLifetime === null) return;
        this.applyRetentionAtThreshold(events, Date.now() - maxLifetime);
    }

    /**
     * Walk every in-memory event in this room (live timeline + alternate timeline sets
     * + thread timelines) and locally redact any whose age exceeds the retention policy.
     *
     * Runs cooperatively: snapshots the timelines up-front then yields to the event loop
     * every {@link Room.RETENTION_SWEEP_YIELD_EVERY} events so the UI thread can paint
     * and handle input between chunks. Each yield also gives React room to coalesce the
     * {@link RoomEvent.Redaction} forceUpdates batched within the previous chunk.
     *
     * Existing event listeners pick up the expiry through the {@link RoomEvent.Redaction}
     * emission that {@link applyEventAsRedaction} already produces, so the timeline UI
     * will re-render expired events as redacted in-place.
     *
     * @returns the number of events that were expired in this sweep.
     */
    public async sweepRetentionPolicy(): Promise<number> {
        const maxLifetime = this.getMaxLifetime();
        if (maxLifetime === null) return 0;

        // Snapshot every timeline's event array up-front. The arrays themselves are
        // internal-to-EventTimeline references, but iteration order within each snapshot
        // is stable across our yields — we only mutate events (via makeRedacted), never
        // the arrays.
        const snapshots: MatrixEvent[][] = [];
        for (const timelineSet of this.timelineSets) {
            for (const timeline of timelineSet.getTimelines()) {
                snapshots.push(timeline.getEvents().slice());
            }
        }
        for (const thread of this.threads.values()) {
            for (const timeline of thread.timelineSet.getTimelines()) {
                snapshots.push(timeline.getEvents().slice());
            }
        }

        // De-duplicate across overlapping sets (a MatrixEvent can live in the main
        // timeline AND in a thread or search timeline set).
        const seen = new Set<string>();
        const expiryThreshold = Date.now() - maxLifetime;
        let expired = 0;
        let sinceYield = 0;
        ...
        for (const events of snapshots) {
            for (const event of events) {
                ...
                if (
                    ...
                    !event.isRedaction() &&
                    !event.isRedacted() &&
                    event.getTs() < expiryThreshold
                ) {
                    const redaction = this.makeRetentionRedaction(event);
                    this.applyEventAsRedaction(redaction, event);
                    expired++;
                }

                if (++sinceYield >= Room.RETENTION_SWEEP_YIELD_EVERY) {
                    sinceYield = 0;
                    // sleep(0) hops to the macrotask queue, letting the browser paint
                    // and React flush the batched forceUpdates from this chunk.
                    await sleep(0);
                }
            }
        }
    ...
    // Tuneable: a larger value reduces yield overhead at the cost of longer main-thread
    // pauses; smaller fragments React renders more. 500 keeps each chunk well under a
    // 16ms frame on typical hardware while still letting React batch within a chunk.
    public static RETENTION_SWEEP_YIELD_EVERY = 500;

    private applyRetentionAtThreshold(events: Iterable<MatrixEvent>, expiryThreshold: number): number {
        let expired = 0;
        for (const event of events) {
            if (event.isState()) continue;
            if (event.isRedaction()) continue;
            if (event.isRedacted()) continue;
            // Use origin_server_ts (getTs) rather than localTimestamp so clock skew on the
            // local machine doesn't cause us to expire events early or late.
            if (event.getTs() >= expiryThreshold) continue;
    ...
    public async addLiveEvents(events: MatrixEvent[], addLiveEventOptions: IAddLiveEventOptions): Promise<void> {
        ...
        // MSC1763: expire any events whose age exceeds the room's retention policy.
        // Done up-front so the rest of the pipeline sees them as already-redacted.
