...
export class MSC4108SignInWithQR {
    ...
    public async loginStep4a(): Promise<void> {
        ...
        if (message?.type === PayloadType.Declined) {
            throw new RendezvousError("Failed", ClientRendezvousFailureReason.UserDeclined);
        }
    ...
    public async loginStep4b(): Promise<DeviceAccessTokenResponse> {
        ...
        if ("error" in res) {
            if (res.error === "expired_token") {
                ...
                throw new RendezvousError(
                    "Declined",
                    ClientRendezvousFailureReason.UserDeclined,
                );
            }
        }
