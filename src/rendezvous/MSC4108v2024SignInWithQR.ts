 ...
 * v2024 messages sent over the secure channel.
 * @experimental Note that this is UNSTABLE and may have breaking changes without notice.
 */
export interface MSC4108v2024Payload {
    type: PayloadType;
}

interface ProtocolsPayload extends MSC4108v2024Payload {
    type: PayloadType.Protocols;
    protocols: string[];
    homeserver: string;
}

interface ProtocolPayload extends MSC4108v2024Payload {
...
interface FailurePayload extends MSC4108v2024Payload {
    type: PayloadType.Failure;
    reason: MSC4108FailureReason;
    homeserver?: string;
}

interface DeclinedPayload extends MSC4108v2024Payload {
    type: PayloadType.Declined;
}

interface SuccessPayload extends MSC4108v2024Payload {
    type: PayloadType.Success;
}

interface AcceptedPayload extends MSC4108v2024Payload {
    type: PayloadType.ProtocolAccepted;
}

interface SecretsPayload
    extends MSC4108v2024Payload, Awaited<ReturnType<NonNullable<CryptoApi["exportSecretsBundle"]>>> {
    type: PayloadType.Secrets;
}

/**
 * Prototype of the unstable [MSC4108](https://github.com/matrix-org/matrix-spec-proposals/pull/4108)
 * v2024 sign in with QR + OIDC flow.
 * @experimental Note that this is UNSTABLE and may have breaking changes without notice.
 */
export class MSC4108v2024SignInWithQR implements MSC4108SignInWithQR {
    ...
    private async receive<T extends MSC4108v2024Payload>(): Promise<T | FailurePayload | undefined> {
        return (await this.channel.secureReceive()) as T | undefined;
    }

    private async send<T extends MSC4108v2024Payload>(payload: T): Promise<void> {
    ...
    public async completeLoginOnNewDevice(_args: { clientId: string }): Promise<DeviceAccessTokenResponse | undefined> {
        throw new Error("Signing in on a new device with the 2024 version of MSC4108 is not supported");
    }
