...
async function testAndBuildPostEndpoint(client: MatrixClient): Promise<string | undefined> {
    try {
        if (await client.doesServerSupportUnstableFeature("io.element.msc4388")) {
            return client.http.getUrl("/io.element.msc4388/rendezvous", undefined, ClientPrefix.Unstable).toString();
        }
    } catch (err) {
...
export class MSC4388RendezvousSession {
    ...
    public constructor({
        ...
        if (id && baseUrl) {
            this.url = `${baseUrl.replace(/\/+$/, "")}/_matrix/client/unstable/io.element.msc4388/rendezvous/${id}`;
        }
    }
    ...
    public async send(data: string): Promise<void> {
        ...
        const responseBody = (await res.json()) as { id?: string; sequence_token: string; expires_in_ms: number };
    ...
    public async receive(): Promise<string | undefined> {
        ...
        while (true) {
            ...
            const body = (await poll.json()) as { data?: string; sequence_token?: string };
