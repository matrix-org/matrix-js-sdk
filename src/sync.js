...
export class SyncApi {
    ...
    createRoom(roomId) {
        ...
        const { timelineSupport, unstableClientRelationAggregation } = client;
    ...
    _registerStateListeners(room) {
        ...
        room.currentState.on("RoomState.newMember", function(
            event,
            state,
            member
        ) {
     ...
     * @private
     */
    _deregisterStateListeners(room) {
        ...
        return client
            ...
            .then(function(data) {
                ...
                if (data.rooms && data.rooms.leave) {
                    leaveRooms = self._mapSyncResponseToRoomArray(
                        data.rooms.leave
                    );
                }
                const rooms = [];
                leaveRooms.forEach(function(leaveObj) {
                    ...
                    const timelineEvents = self._mapSyncEventsFormat(
                        leaveObj.timeline,
                        room
                    );
                    const stateEvents = self._mapSyncEventsFormat(
                        leaveObj.state,
                        room
                    );
            ...
            if (response.presence && utils.isArray(response.presence)) {
                response.presence
                    ...
                    .forEach(function(presenceEvent) {
                        let user = client.store.getUser(
                            presenceEvent.getContent().user_id
                        );
                        if (user) {
            ...
            .then(
                function(res) {
                    ...
                    res.chunk
                        ...
                        .forEach(function(presenceEvent) {
                            let user = self.client.store.getUser(
                                presenceEvent.getContent().user_id
                            );
                            if (user) {
                                user.setPresenceEvent(presenceEvent);
    ...
    async _wasLazyLoadingToggled(lazyLoadMembers) {
    ...
    sync() {
        ...
        const checkLazyLoadStatus = async () => {
            ...
            if (this.opts.lazyLoadMembers) {
                ...
                if (supported) {
                    try {
                        debuglog(
                            "Creating and storing lazy load sync filter..."
                        );
                    ...
                    debuglog(
                        "LL: lazy loading requested but not supported " +
            ...
            if (shouldClear) {
                ...
                logger.warn(
                    "InvalidStoreError: store is not usable: stopping sync."
                );
                return;
            }
        ...
        if (client.isGuest()) {
            ...
            savedSyncPromise = client.store
                ...
                .then(savedSync => {
                    debuglog(
                        `Got reply from saved sync, exists? ${!!savedSync}`
                    );
                    if (savedSync) {
     ...
     */
    async _syncFromCache(savedSync) {
        debuglog(
    ...
    async _sync(syncOptions) {
        ...
        try {
            ...
            if (this._currentSyncRequest === null) {
                this._currentSyncRequest = this._doSyncRequest(
                    syncOptions,
                    syncToken
                );
            }
    ...
    _doSyncRequest(syncOptions, syncToken) {
    ...
    _getSyncParams(syncOptions, syncToken) {
        ...
        if (
            this.getSyncState() == "ERROR" ||
            this.getSyncState() == "RECONNECTING"
        ) {
     ...
     */
    async _processSyncResponse(syncEventData, data) {
        const client = this.client;
        const self = this;
        // data looks like:
        // {
        ...
        if (data.account_data && utils.isArray(data.account_data.events)) {
            const events = data.account_data.events.map(
                client.getEventMapper()
            );
        ...
        if (
            data.to_device &&
            utils.isArray(data.to_device.events) &&
            ...
            data.to_device.events
                ...
                .map(toDeviceEvent => {
                    ...
                    // below.
                    if (
                        toDeviceEvent.getType() === "m.key.verification.cancel"
                    ) {
                        const txnId = toDeviceEvent.getContent()[
                            "transaction_id"
                        ];
                        if (txnId) {
        ...
        if (data.rooms) {
            if (data.rooms.invite) {
                inviteRooms = this._mapSyncResponseToRoomArray(
                    data.rooms.invite
                );
            }
            if (data.rooms.join) {
        ...
        await utils.promiseMapSeries(joinRooms, async function(joinObj) {
            ...
            const ephemeralEvents = self._mapSyncEventsFormat(
                joinObj.ephemeral
            );
            const accountDataEvents = self._mapSyncEventsFormat(
                joinObj.account_data
            );
        ...
        if (this.opts.crypto && data.device_one_time_keys_count) {
            const currentCount =
                data.device_one_time_keys_count.signed_curve25519 || 0;
     ...
     */
    _processGroupSyncEntry(groupsSection, sectionName) {
     ...
     * @return {Object[]}
     */
    _mapSyncResponseToRoomArray(obj) {
     ...
     */
    _processRoomEvents(room, stateEventList, timelineEventList, fromCache) {
     ...
     * is earlier in time. Higher index is later.
     */
    _processEventsForNotifs(room, timelineEventList) {
        // gather our notifications into this._notifEvents
        if (this.client.getNotifTimelineSet()) {
            for (let i = 0; i < timelineEventList.length; i++) {
                const pushActions = this.client.getPushActionsForEvent(
                    timelineEventList[i]
                );
                if (
                    pushActions &&
