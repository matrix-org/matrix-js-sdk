...
export class TimelineWindow {
     ...
     * @return {Promise}
     */
    load(initialEventId, initialWindowSize) {
        const self = this;
        ...
        const initFields = function(timeline) {
            ...
            self._start = new TimelineIndex(
                timeline,
                startIndex - timeline.getBaseIndex()
            );
            self._end = new TimelineIndex(
                timeline,
                endIndex - timeline.getBaseIndex()
            );
        ...
        //
        if (initialEventId) {
            const timeline = this._timelineSet.getTimelineForEvent(
                initialEventId
            );
            if (timeline) {
            ...
            const prom = this._client.getEventTimeline(
                this._timelineSet,
                initialEventId
            );
    ...
    extend(direction, size) {
        ...
        if (count) {
            ...
            debuglog(
                "TimelineWindow: increased cap by " +
                    count +
                    " (now " +
                    this._eventCount +
                    ")"
    ...
    canPaginate(direction) {
        ...
        return Boolean(
            tl.timeline.getNeighbouringTimeline(direction) ||
     ...
     */
    paginate(direction, size, makeRequest, requestLimit) {
