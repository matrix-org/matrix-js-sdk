...
export class MatrixCall extends EventEmitter {
        ...
        if (stream instanceof MediaStream) {
            ...
            if (videoEl && this.type == "video") {
                ...
                if (this.screenSharingStream) {
                    debuglog(
                        "Setting screen sharing stream to the local video" +
                            " element"
                    );
                    this.assignElement(
                        videoEl,
            ...
            debuglog(
                "User denied access to camera/microphone." +
    ...
    _sendAnswer(stream) {
        sendEvent(this, "m.call.answer", this._answerContent)
            .then(() => {
    ...
    _onIceConnectionStateChanged() {
        ...
        debuglog(
            "Ice connection state changed to: " +
                this.peerConn.iceConnectionState
        );
        // ideally we'd consider the call to be connected when we get media but
        // chrome doesn't implement any of the 'onstarted' events yet
        if (
            this.peerConn.iceConnectionState == "completed" ||
    ...
    _onAddStream(event) {
        ...
        if (event.stream.oninactive !== undefined) {
            event.stream.oninactive = hookCallback(
                self,
                self._onRemoteStreamEnded
            );
        } else {
            // onended is deprecated from Chrome 54
            event.stream.onended = hookCallback(
                self,
                self._onRemoteStreamEnded
            );
        }
        // not currently implemented in chrome
        event.stream.onstarted = hookCallback(
            self,
            self._onRemoteStreamStarted
        );
        if (this.type === "video") {
            _tryPlayRemoteStream(this);
            _tryPlayRemoteAudioStream(this);
...
const _sendCandidateQueue = function(self) {
    ...
    sendEvent(self, "m.call.candidates", content).then(
        function() {
...
const _createPeerConnection = function(self) {
    ...
    pc.oniceconnectionstatechange = hookCallback(
        self,
        self._onIceConnectionStateChanged
    );
    pc.onsignalingstatechange = hookCallback(
        self,
        self._onSignallingStateChanged
    );
...
export function createNewMatrixCall(client, roomId, options) {
    ...
    try {
        ...
        logger.error(
            "Failed to set up WebRTC object: possible browser interference?"
        );
        logger.error(e);
        return null;
    }
